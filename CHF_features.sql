drop materialized view if exists all_data_readmission;
create materialized view all_data_readmission as
  with lab_aggs as
  (
	...
	, min(case when le.itemid=50983 then le.valuenum else null end) as sodium
    from labevents le
  ...
  (
    ...
    , min(case when itemid in (814, 220228) then valuenum else null end) as hemoglobin
	from chartevents
    where itemid in
    (
